...
const Badge = ({ children, action = 'muted', variant = 'solid', size = 'md', className, ...props }: { className?: string } & IBadgeProps) => {
  return (
    <ContextView
...
const BadgeIcon = React.forwardRef<React.ElementRef<typeof UIIcon>, IBadgeIconProps>(({ className, size, ...props }, ref) => {
  const { size: parentSize, action: parentAction } = useStyleContext(SCOPE);
